# Documentation Index

## Release Notes

- [v0.6.0 (release checklist)](./releases/v0.6.0.md)
- [v0.5.3 (planned)](./releases/v0.5.3.md)
- [v0.5.2](./releases/v0.5.2.md)
- [v0.5.1](./releases/v0.5.1.md)
- [v0.5.0](./releases/v0.5.0.md)
- [v0.4.8](./releases/v0.4.8.md)
- [v0.4.7](./releases/v0.4.7.md)

## Testing & Validation

- [Complex failure end-to-end scenario](./testing/complex-failure-e2e.md)
- [v0.6.0 Day 4 UI validation](./testing/v0.6.0-day4-ui.md)
- [v0.6.0 Day 5 cluster validation](./testing/v0.6.0-day5-cluster-validation.md)

## Visual Assets

- UI screenshots and release illustrations: [`docs/images/`](./images)
